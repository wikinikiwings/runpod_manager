...
### `pod_image_*` (выбор образа пода)

Три ключа в `admin_settings.json`, управляемые блоком «🖼 Образы подов» в
админ-панели. Позволяют назначить каждому проекту свой RunPod-шаблон.

- `pod_image_catalog` — упорядоченный список записей `{label, template_id}`;
  определяет набор образов, доступных для выбора. Засевается одной записью
  `{label:"Текущий (comfy_runpod)", template_id:"i3j2sm66q8"}`, что
  гарантирует неизменность поведения до первого действия админа.
- `default_pod_image` — `template_id` дефолтной записи каталога. Применяется
  к проектам без явного выбора, unassigned-подам и admin-подам.
- `project_pod_image` — dict `{project: template_id}`. Отсутствие ключа для
  проекта означает «использовать дефолт».

**Резолвер `resolve_template_id(project)`** (Settings-секция `runpod_manager.py`):
приоритет — явный выбор проекта (если запись ещё в каталоге) → глобальный
дефолт → `PRESET["template_id"]` как страховка при пустом/сломанном каталоге.

**POST-валидация (`compute_image_settings_update`):**
- `pod_image_catalog`: каждая запись — непустая `label` (≤ 60 символов после strip),
  `template_id` — непустая строка, матчит `^[A-Za-z0-9_-]+$`; дубли по
  `template_id` отбрасываются (остаётся первая); если после фильтрации список
  пуст — сохраняется прежний каталог (каталог не может опустеть).
- `default_pod_image`: должен существовать в (новом) каталоге; иначе берётся
  `template_id` первого элемента каталога.
- `project_pod_image`: принимаются только ключи из `PROJECTS`; значение должно
  быть в каталоге, иначе ключ не сохраняется (проект уходит на дефолт).

**GET `/api/admin/settings`** — возвращает все три ключа; если `pod_image_catalog`
пуст в настройках, в ответ подставляется каталог с дефолтной записью из
`DEFAULT_SETTINGS` (бэкфилл без записи в файл).

**UI-блок «🖼 Образы подов»:**
- **Редактор каталога** — строки `[название] [template_id] [⭐дефолт] [🗑]`
  + кнопка «+ Добавить образ». Ровно одна запись помечена дефолтом
  (радио/звёздочка). Кнопка 🗑 дефолтной строки заблокирована (нельзя удалить
  дефолтный образ).
- **Сетка per-project** — на каждый `PROJECTS` `<select>` с понятными названиями
  из каталога; первый пункт «По умолчанию (`<label дефолта>`)» соответствует
  отсутствию ключа в `project_pod_image`. В дропдаун попадают только образы с
  заданным `label` (запись без названия нельзя назначить — иначе при сохранении
  она выпала бы из каталога и назначение потерялось бы).
- Сохранение — в общий `POST /api/admin/settings` вместе с прочими настройками
  (тот же паттерн что у `project_quotas`).

**Образ привязан к проекту создаваемого пода** (ключевой момент). При деплое
`resolve_template_id(project)` берёт `project_pod_image[project]` → дефолт →
`PRESET`. Поэтому:
- **обычный пользователь** — `project` берётся из сессии, образ его проекта
  применяется автоматически;
- **админ** — `project` берётся из дропдауна «+ New Pod» (`assigned_project`).
  Дефолтная опция — «ADMIN (мой проект)» (`assigned_project=ADMIN`); «Не назначать»
  (`__null__`) даёт unassigned-под на дефолтном образе. Чтобы под проекта CV
  поднялся на образе CV, админ должен выбрать CV в дропдауне. Рядом показывается
  хинт «образ: `<label>`» (данные — поле `imageByProject` в ответе `/api/pods`),
  отражающий, какой образ применится для текущего выбора.

Выбранный `template_id` используется в `DeployOnDemand` (GraphQL) и в CLI-fallback
(`--template-id`/`--templateId`) при каждом деплое и каждой попытке авторетрая.
